    ...
    public class Transformation : UntypedActor
    {
        ...
        protected override void OnReceive(object message)
        {
            ...
            if (message is TransformationMessages.TransformationJob)
            {
                ...
                Sender.Tell(
                    new TransformationMessages.TransformationResult(
                        $"[{Self.Path.ToStringWithAddress(Cluster.SelfAddress)}]{job.ToString().ToUpper()}"), Self);
            }
        ...
        {
            if (member.HasRole("connector"))
            {
                Console.WriteLine("connector callback");
                Console.WriteLine(Sender);
                Console.WriteLine("====connector callback");
                
                Context.ActorSelection(member.Address + "/user/connector")
                    .Tell(TransformationMessages.BACKEND_REGISTRATION, Self);
            }
        }
    }
}